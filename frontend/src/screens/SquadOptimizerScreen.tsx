...
import bouLogo from "../assets/pics/EPL Logos/bournemouth.football-logos.cc.png";
import breLogo from "../assets/pics/EPL Logos/brentford.football-logos.cc.png";
...
import cryLogo from "../assets/pics/EPL Logos/crystal-palace.football-logos.cc.png";
import eveLogo from "../assets/pics/EPL Logos/everton.football-logos.cc.png";
import fulLogo from "../assets/pics/EPL Logos/fulham.football-logos.cc.png";
...
import nfoLogo from "../assets/pics/EPL Logos/nottingham-forest.football-logos.cc.png";
...
import souLogo from "../assets/pics/EPL Logos/sunderland.football-logos.cc.png"; // Fallback for SOU if missing
import leiLogo from "../assets/pics/EPL Logos/leeds-united.football-logos.cc.png"; // Fallback for LEI
import ipsLogo from "../assets/pics/EPL Logos/burnley.football-logos.cc.png"; // Fallback for IPS

// PSL franchise team colours used as avatar badges
const PSL_TEAM_COLORS: Record<string, { bg: string; text: string }> = {
  KRK: { bg: '#00BFFF22', text: '#00BFFF' },
  LHQ: { bg: '#DC143C22', text: '#DC143C' },
  ISU: { bg: '#0055BE22', text: '#0055BE' },
  PZL: { bg: '#FFD70022', text: '#FFD700' },
  QTG: { bg: '#80008022', text: '#CC77CC' },
  MLS: { bg: '#FF8C0022', text: '#FF8C00' },
};

const PL_TEAM_COLORS: Record<string, { bg: string; text: string }> = {
  ARS: { bg: '#EF010722', text: '#EF0107' }, AVL: { bg: '#95BFE522', text: '#95BFE5' },
  BOU: { bg: '#B50E1222', text: '#B50E12' }, BRE: { bg: '#E3061322', text: '#E30613' },
  BHA: { bg: '#0057B822', text: '#0057B8' }, CHE: { bg: '#03469422', text: '#034694' },
  CRY: { bg: '#1B458F22', text: '#1B458F' }, EVE: { bg: '#00339922', text: '#003399' },
  FUL: { bg: '#FFFFFF22', text: '#FFFFFF' }, IPS: { bg: '#0000FF22', text: '#0000FF' },
  LEI: { bg: '#00309022', text: '#003090' }, LIV: { bg: '#C8102E22', text: '#C8102E' },
  MCI: { bg: '#6CABDD22', text: '#6CABDD' }, MUN: { bg: '#DA291C22', text: '#DA291C' },
  NEW: { bg: '#00000022', text: '#FFFFFF' }, NFO: { bg: '#DD000022', text: '#DD0000' },
  SOU: { bg: '#D7192022', text: '#D71920' }, TOT: { bg: '#13225722', text: '#132257' },
  WHU: { bg: '#7A263A22', text: '#7A263A' }, WOL: { bg: '#FDB91322', text: '#FDB913' },
};
...
const PSL_CLUBS = [
  { code: 'KRK', name: 'Karachi Kings' },
  { code: 'LHQ', name: 'Lahore Qalandars' },
  { code: 'ISU', name: 'Islamabad United' },
  { code: 'PZL', name: 'Peshawar Zalmi' },
  { code: 'QTG', name: 'Quetta Gladiators' },
  { code: 'MLS', name: 'Multan Sultans' },
];

const PL_CLUBS = [
  { code: 'ARS', name: 'Arsenal', logo: arsLogo },
  { code: 'AVL', name: 'Aston Villa', logo: avlLogo },
  { code: 'BOU', name: 'Bournemouth', logo: bouLogo },
  { code: 'BRE', name: 'Brentford', logo: breLogo },
  { code: 'BHA', name: 'Brighton', logo: bhaLogo },
  { code: 'CHE', name: 'Chelsea', logo: cheLogo },
  { code: 'CRY', name: 'Crystal Palace', logo: cryLogo },
  { code: 'EVE', name: 'Everton', logo: eveLogo },
  { code: 'FUL', name: 'Fulham', logo: fulLogo },
  { code: 'IPS', name: 'Ipswich', logo: ipsLogo },
  { code: 'LEI', name: 'Leicester', logo: leiLogo },
  { code: 'LIV', name: 'Liverpool', logo: livLogo },
  { code: 'MCI', name: 'Man City', logo: mciLogo },
  { code: 'MUN', name: 'Man Utd', logo: munLogo },
  { code: 'NEW', name: 'Newcastle', logo: newLogo },
  { code: 'NFO', name: "Nottingham", logo: nfoLogo },
  { code: 'SOU', name: 'Southampton', logo: souLogo },
  { code: 'TOT', name: 'Tottenham', logo: totLogo },
  { code: 'WHU', name: 'West Ham', logo: whuLogo },
  { code: 'WOL', name: 'Wolves', logo: wolLogo },
];

export const SquadOptimizerScreen: React.FC = () => {
  const { 
  ...
  const [injectData, setInjectData] = useState({
    name: '',
    club: sportContext === 'cricket' ? 'KRK' : 'ARS',
    position: sportContext === 'cricket' ? 'BAT' : 'MID',
    cost_millions: 8.0,
    overall_ability: 80,
    base_form: 75,
    expectation_status: 'High Expectation'
  });
  ...
  const activeClubs = sportContext === 'cricket' ? PSL_CLUBS : PL_CLUBS;
  const activeColors = sportContext === 'cricket' ? PSL_TEAM_COLORS : PL_TEAM_COLORS;

  // Sync Inject form defaults when sport changes
  useEffect(() => {
    if (sportContext === 'cricket') {
      setInjectData({
        name: '', club: 'KRK', position: 'BAT',
        cost_millions: 8.0, overall_ability: 80, base_form: 75, expectation_status: 'High Expectation'
      });
    } else {
      setInjectData({
        name: '', club: 'ARS', position: 'MID',
        cost_millions: 8.0, overall_ability: 80, base_form: 75, expectation_status: 'High Expectation'
      });
    }
  }, [sportContext]);
  ...
  const handleInjectSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    addCustomPlayer(injectData as CustomPlayerInput);
  ...
  const { starters, dynamicFormation } = useMemo(() => {
    ...
    // Exactly 11 players for the squad.
    const squadStarters = squad.slice(0, 11);
    const coords: { left: string, top: string }[] = [];
    
    if (sportContext === 'cricket') {
      const wks = squadStarters.filter(p => p?.position === 'WK');
      const bats = squadStarters.filter(p => p?.position === 'BAT');
      const ars = squadStarters.filter(p => p?.position === 'AR');
      const bwls = squadStarters.filter(p => p?.position === 'BWL');
      
      const finalStarters = [...wks, ...bats, ...ars, ...bwls];
      
      // 1. Wicket Keeper (Top center)
      wks.forEach((_, i) => {
        const total = wks.length;
        const spacing = 40 / Math.max(1, total);
        const start = 50 - (spacing * (total - 1)) / 2;
        coords.push({ left: `${start + (i * spacing)}%`, top: '10%' });
      });
      
      // 2. Batters (Top-middle row)
      const batTop = '35%';
      bats.forEach((_, i) => {
        const total = bats.length;
        const spacing = 85 / Math.max(1, total);
        const start = 50 - (spacing * (total - 1)) / 2;
        coords.push({ left: `${start + (i * spacing)}%`, top: batTop });
      });
      
      // 3. All-Rounders (Bottom-middle row)
      const arTop = '62%';
      ars.forEach((_, i) => {
        const total = ars.length;
        const spacing = 60 / Math.max(1, total);
        const start = 50 - (spacing * (total - 1)) / 2;
        coords.push({ left: `${start + (i * spacing)}%`, top: arTop });
      });
      
      // 4. Bowlers (Bottom row)
      const bwlTop = '88%';
      bwls.forEach((_, i) => {
        const total = bwls.length;
        const spacing = 80 / Math.max(1, total);
        const start = 50 - (spacing * (total - 1)) / 2;
        coords.push({ left: `${start + (i * spacing)}%`, top: bwlTop });
      });
      return { starters: finalStarters, dynamicFormation: coords };
    } else {
      // Football Formation (Parity with Battle-Tested Logic)
      ...
      // Fallback GK logic for football
      if (gks.length === 0) {
  ...
  const handleGwChange = (newGw: number) => {
    const maxGw = sportContext === 'cricket' ? 14 : 38;
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // PRIORITY 1: Check location.state for fixtures (direct from Generator)
      if (location.state?.seasonalFixtures) {
        useFplStore.setState({ seasonalFixtures: location.state.seasonalFixtures });
        hasFixtures = true;
      }

      // PRIORITY 2: Check localStorage for the LATEST saved fixtures.
      if (!hasFixtures) {
        const history = getHistory();
        const latestMatch = history.find(h => 
          (sportContext === 'cricket' && (h.source === 'Pakistan Super League' || h.name === 'Pakistan Super League' || h.source === 'psl')) ||
          (sportContext === 'football' && (h.source === 'Premier League' || h.name === 'Premier League' || h.source === 'pl'))
        );
        if (latestMatch) {
          useFplStore.setState({ seasonalFixtures: { fixtures: latestMatch.fixtures, teams: latestMatch.teams } });
          hasFixtures = true;
        }
      }

      // PRIORITY 3: Check appStore for generated fixtures (if any)
      const appStore = useAppStore.getState();
      if (!hasFixtures && appStore.generatedFixtures) {
        useFplStore.setState({ seasonalFixtures: appStore.generatedFixtures });
        hasFixtures = true;
      }

      // PRIORITY 4: Backend-generated fixtures (last resort)
      if (!hasFixtures) {
    ...
    init();
  }, [location, setOptimizationResult, sportContext]);

  useEffect(() => {
    if (sportContext === 'cricket') {
      setInjectData(f => ({ ...f, club: 'KRK', position: 'BAT' }));
    } else {
      setInjectData(f => ({ ...f, club: 'ARS', position: 'FWD' }));
    }
  }, [sportContext]);
    ...
    <>
      ...
      <div className="relative z-10 grid gap-8 lg:gap-12 lg:grid-cols-[280px_1fr] xl:grid-cols-[320px_1fr] max-w-[2400px] mx-auto p-4 sm:p-8 md:p-16">
        ...
        <aside className="space-y-6">
          <FilterPanel title={sportContext === 'cricket' ? "PSL TEAMS" : "PL TEAMS"}>
             <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-3 gap-3 sm:gap-4 max-h-[300px] overflow-y-auto pr-2">
               {activeClubs.map((team: any) => {
                 const colors = activeColors[team.code] || { bg: '#ffffff11', text: '#aaaaaa' };
                 return (
                   <div key={team.code}
                     className="flex h-12 sm:h-16 items-center justify-center rounded-xl sm:rounded-2xl border border-white/5 p-2 sm:p-3 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-500 hover:border-emerald-500/30 gap-1"
                     style={{ backgroundColor: sportContext === 'cricket' ? colors.bg : 'rgba(0,0,0,0.4)' }}
                   >
                     {team.logo ? (
                       <img src={team.logo} alt={team.code} className="h-full w-full object-contain" title={team.name} />
                     ) : (
                       <span className="text-2xl font-black" style={{ color: colors.text }}>{team.code}</span>
                     )}
                   </div>
                 );
               })}
        ...
        <main className="space-y-8 sm:space-y-12">
          ...
          <header className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-center lg:items-end justify-between px-2 sm:px-4 w-full">
                  ...
                  <div className="mt-4 xl:mt-6 flex items-center justify-center gap-3 lg:gap-6 bg-black/40 border border-emerald-500/20 px-4 lg:px-6 py-2 xl:py-3 rounded-[16px] xl:rounded-[24px] w-fit shadow-[0_0_20px_rgba(16,185,129,0.1)] backdrop-blur-md">
                     <button onClick={() => handleGwChange(gameweek - 1)} className="text-zinc-400 hover:text-emerald-400 transition-all hover:scale-125 shrink-0">
                      ...
                      <div className="flex flex-col items-center min-w-[70px] lg:min-w-[100px] shrink-0">
                        <span className="text-xl lg:text-2xl font-black text-white uppercase italic tracking-tighter drop-shadow-md">
                          {sportContext === 'cricket' ? 'MATCH' : 'GW'} {gameweek}
                        </span>
                     </div>
                     <button onClick={() => handleGwChange(gameweek + 1)} className="text-zinc-400 hover:text-emerald-400 transition-all hover:scale-125 shrink-0">
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-stretch sm:items-center justify-center lg:justify-end shrink-0 w-full lg:w-auto mt-2 lg:mt-0">
               ...
               <button 
                onClick={() => {
                   const newSport = sportContext === 'cricket' ? 'football' : 'cricket';
                   setSportContext(newSport);
                   setTimeout(() => optimize(false), 100);
                }}
                className="px-6 sm:px-8 py-5 lg:py-5 rounded-[20px] xl:rounded-[32px] bg-black/40 border border-emerald-500/30 text-white font-black text-sm uppercase tracking-widest hover:bg-emerald-500/10 transition-all flex flex-col items-center justify-center gap-1 shrink-0 flex-1 sm:flex-none shadow-[0_0_30px_rgba(16,185,129,0.1)] backdrop-blur-md"
               >
                 <span className="text-[10px] text-zinc-400 tracking-[0.2em] font-bold">SPORT</span>
                 <span className="text-lg italic tracking-tighter leading-none">{sportContext === 'cricket' ? 'CRICKET' : 'FOOTBALL'}</span>
               </button>
                  ...
                  <div className="p-6 sm:p-10">
                    ...
                    <div className="flex items-center gap-5 mb-6">
                      ...
                      <div className="flex-1">
                        ...
                        <input 
                          type="text" 
                          value={injectData.name} 
                          onChange={e => setInjectData(f => ({...f, name: e.target.value}))} 
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div>
                        <label className="text-[8px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-1 block">Franchise / Club</label>
                        <select value={injectData.club} onChange={e => setInjectData(f => ({...f, club: e.target.value}))} className="w-full bg-white/[0.03] border border-white/8 rounded-xl py-2.5 px-3 text-white text-sm font-bold focus:outline-none focus:ring-1 focus:ring-purple-500/50 appearance-none cursor-pointer">
                          {activeClubs.map((c: {code: string, name: string}) => <option key={c.code} value={c.code} className="bg-[#0a0a0c] text-white">{c.name}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="text-[8px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-1 block">Role</label>
                        <select value={injectData.position} onChange={e => setInjectData(f => ({...f, position: e.target.value as any}))} className="w-full bg-white/[0.03] border border-white/8 rounded-xl py-2.5 px-3 text-white text-sm font-bold focus:outline-none focus:ring-1 focus:ring-purple-500/50 appearance-none cursor-pointer">
                          {sportContext === 'cricket' ? 
                            ['WK','BAT','AR','BWL'].map(p => <option key={p} value={p} className="bg-[#0a0a0c] text-white">{p === 'WK' ? 'Wicketkeeper' : p === 'BAT' ? 'Batsman' : p === 'AR' ? 'All-Rounder' : 'Bowler'}</option>)
                            :
                            ['GK','DEF','MID','FWD'].map(p => <option key={p} value={p} className="bg-[#0a0a0c] text-white">{p === 'GK' ? 'Goalkeeper' : p === 'DEF' ? 'Defender' : p === 'MID' ? 'Midfielder' : 'Forward'}</option>)
                          }
                        </select>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-6">
                      <div className="bg-white/[0.02] rounded-xl p-4 border border-white/5">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-sm font-black text-purple-400 tabular-nums">{injectData.cost_millions.toFixed(1)}M</span>
                        </div>
                        <input type="range" min={4.0} max={15.0} step={0.1} value={injectData.cost_millions} onChange={e => setInjectData(f => ({...f, cost_millions: parseFloat(e.target.value)}))} className="w-full accent-purple-500 h-1.5 rounded-full bg-white/5 cursor-pointer" />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-white/[0.02] rounded-xl p-4 border border-white/5">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-sm font-black text-purple-400 tabular-nums">{injectData.overall_ability}</span>
                          </div>
                          <input type="range" min={50} max={100} step={1} value={injectData.overall_ability} onChange={e => setInjectData(f => ({...f, overall_ability: parseInt(e.target.value)}))} className="w-full accent-purple-500 h-1.5 rounded-full bg-white/5 cursor-pointer" />
                        </div>
                        <div className="bg-white/[0.02] rounded-xl p-4 border border-white/5">
                          <div className="flex justify-between items-center mb-2">
                            <label className="text-[8px] font-black text-zinc-500 uppercase tracking-[0.2em]">Form</label>
                            <span className="text-sm font-black text-purple-400 tabular-nums">{injectData.base_form.toFixed(1)}</span>
                          </div>
                          <input type="range" min={1.0} max={10.0} step={0.1} value={injectData.base_form} onChange={e => setInjectData(f => ({...f, base_form: parseFloat(e.target.value)}))} className="w-full accent-purple-500 h-1.5 rounded-full bg-white/5 cursor-pointer" />
                        </div>
                      </div>
 
                      <div>
                        <label className="text-[8px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-1 block">Expectation Status</label>
                        <select value={injectData.expectation_status} onChange={e => setInjectData(f => ({...f, expectation_status: e.target.value}))} className="w-full bg-white/[0.03] border border-white/8 rounded-xl py-2.5 px-3 text-white text-sm font-bold focus:outline-none focus:ring-1 focus:ring-purple-500/50 appearance-none cursor-pointer">
