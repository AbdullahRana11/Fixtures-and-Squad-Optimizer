...
// Standard league ΓåÆ required top-tier league name
...
export const saveTournament = async (req: Request, res: Response) => {
  try {
    ...
    // --- Core upsert (original fields only ΓÇö always safe) ---
      ...
      try {
        ...
        if (plTournament) {
          ...
          if (rescheduleResult.changes.length > 0) {
            ...
            // Log rescheduling actions (non-fatal ΓÇö table may not exist)
            try {
              for (const change of rescheduleResult.changes) {
                await prisma.reschedulingLog.create({
                  data: {
