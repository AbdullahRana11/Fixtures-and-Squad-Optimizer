...
export const optimizeSquad = async (req: Request, res: Response) => {
  try {
    const { budget, gameweek, k_index = 1, customPlayers = [], sport = 'football' } = req.body;
    ...
    const engine = new FPLKnapsackEngine(fullPool, budget, k_index, sport);
    const result = engine.optimize();

    if (result.squad.length !== 11) {
       return res.status(400).json({ 
         error: { 
           message: `Impossible constraint or algorithm timed out before finding a valid 11-player ${sport} XI within budget.`,
...
export const constrainedSwap = async (req: Request, res: Response) => {
  try {
    const { remaining_budget, required_position, exclude_clubs, sport = 'football' } = req.body;
